...
The runLab property is expected to be a function containing the student's code. The runLab function is called and the result is compared to the expected result. If the result and expected results don't match, then the lab is considered a failure.
gradeLabs
...
function gradeLabs(arr) {
  for (let i = 0; i < arr.length; i++) {
    try {
      if (arr[i].runLab(5) === 25) {
        throw "result is true";
      } else {
        throw "result is failed";
      }
    } catch (e) {
      console.log(e);
    }
